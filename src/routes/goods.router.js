...
// 제품 글 수정
router.patch('/goods/:goodsId/content', authMiddleware, async (req, res, next) => {
    try {
        const { userId } = req.user;
...
// 제품 글 삭제
router.delete('/goods/:goodsId/content', authMiddleware, async (req, res, next) => {
    try {
        const { userId } = req.user;
        const { goodsId } = req.params;
...
router.post('/goods/:goodsId/like', authMiddleware, async (req, res, next) => {
    try {
        ...
        let isLike = await prisma.likes.findFirst({ where: { GoodsId: +goodsId, UserId: +userId } });
